...
We develop the informatics infrastructure for phenotype data across species and clinical settings: phenotype ontologies (HPO, MP, ZP, FLOPO, plant traits), cross-species phenotype crosswalks, tools that capture and standardize phenotype descriptions and computational pipelines that link phenotype data to underlying genes, variants and diseases. (rkg: topics/phenotype-informatics.md) The diagnosis of rare diseases requires the integration of patient-specific data with large-scale background knowledge, such as the Human Phenotype Ontology (HPO). (rkg: topics/rare-disease-diagnostic-support.md) We develop systems like PhenomeNET and PVP that use automated reasoning and machine learning to prioritize disease-causing genomic variants based on their phenotypic consequences. (rkg: topics/rare-disease-diagnostic-support.md)
